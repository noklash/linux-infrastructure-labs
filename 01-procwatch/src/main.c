...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "p:t:Tf:m:i:l:h", longopts, NULL)) != -1) {
        ...
        case 't': sort_by = optarg; break;
        case 'T': do_tree = 1; break;
        case 'f': opt_fds = atoi(optarg); break;
        case 'm': opt_maps = atoi(optarg); break;
        case 'i': interval = atof(optarg); if (interval < 0.1) interval = 0.1; break;
    ...
    if (opt_pid > 0) {
        ...
        unsigned long long delta = (b.utime + b.stime) - (a.utime + a.stime);
    ...
    if (do_tree) {
        ...
        if (strcmp(sort_by, "memory") == 0 || strcmp(sort_by, "mem") == 0)
            proc_sort_mem(&curr);
        else
            proc_sort_cpu(&curr);
